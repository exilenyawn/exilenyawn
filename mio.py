...
#Well, you can actually take a little time to look at it and learn something
  ...
  def onMessage(self, room, user, message):
   ...
   try:
    ...
    if "belum?" in message.body.lower() and "^" not in message.body[:1] and self.getAccess(user) >=1 and not user.name in blacklist and not room.name in simlock:
        if len(args) > 1:
            room.message(__import__("simi").simi(args),True)
    if "yuk?" in message.body.lower() and "^" not in message.body[:1] and self.getAccess(user) >=1 and not user.name in blacklist and not room.name in simlock:
        if len(args) > 1:
            room.message(__import__("simi").simi(args),True)
    if ",," in message.body.lower() and "^" not in message.body[:1] and self.getAccess(user) >=1 and not user.name in blacklist and not room.name in simlock:
        if len(args) > 1:
            room.message(__import__("simi2").simi(args),True)
    ...
    if message.body.startswith("Test") or message.body.startswith("test"):
     if self.getAccess(user) >= 1 and not user.name in blacklist:      
      room.message(("<f x12ff0000='1'><br/>Test Request By : @"+user.name+" <f x12ff99ff='1'><br/>Recived By HarukazeMio."),True)
    if message.body.startswith("Haruka") or message.body.startswith("haruka"):
     if self.getAccess(user) >= 5 or room.getLevel(user) > 0 and not user.name in blacklist:
      jawab = ["Ya, Kak","Ada apa kakak?"]
      room.message(random.choice(jawab)+" @"+user.name+" :D")
     else:
       room.message("Siapa yah?")
    if message.body.startswith("Natsuru mana?"):
     if self.getAccess(user) >= 1 or room.getLevel(user) > 0 and not user.name in blacklist:
      room.message("Natsurunya lagi pulang kampung XD")
   ...
   ## Reg
      if cmd == "reg":
        ...
        if name not in whitelist and name not in player and name not in archwizard and name not in owners and name not in admin and name not in archknight and name not in blacklist:
          room.message(user.name+" sekarang kamu dapat menggunakanku , ketik ^cmds untuk perintah :)")
    ...
    ##check access and ignore
      ...
      if cmd == "lockstatus":
        if args:
          if args in locks:
            roomlock = "<f x11FF0000='0'>Cmd is Locked</f>"
          else:
            roomlock = "<f x1133FF33='0'>Cmd is Unlocked</f>"
          if args in simlock:
            roomlock2 = "<f x11FF0000='0'> Simsimi is Locked</f>"
          else:
            roomlock2 = "<f x1133FF33='0'>Simsimi is Unlocked</f>"
          room.message("<br/> [Lockstatus of "+args+"] <br/>"+roomlock+"<br/>"+roomlock2,True)          
        if not args:
          ...
          if room.name in simlock:
            ...
            roomlock2 = "<f x1133FF33='0'>Simsimi is Unlocked</f>"
          room.message("<br/> [Lockstatus] <br/>"+roomlock+"<br/>"+roomlock2,True)
      ...
      ##cmds
        ...
        if user.name in owners and not user.name in admin and not user.name in archknight and not user.name in whitelist:
          room.message("<br/>"+user.name+" Rank 6 [Owner] "+"<br/>"+" Perintah[ ^ atau * ] :<br/>rb , rb2 , wl , whois , webanime , cso , pfpic , mini , prof of profile , gs(Google search) , yt(Youtube) , df(define) , udf(undefine) , fax , bc , sn(sendnote) , rn(readnote) , join , leave , mydict , nick , staff , setnick , mynick , seenick , profile , rank , myrank , ranker , clear , del , sf , sfc, snc , sfz , myip , mc , mc2 , lock , unlock , lockstatus",True) 
          self.getRoom("mio-chat").message("<br/><br/><b>Nama</b>: %s <br/><b>Rooms</b>: %s  <br/><b>Command</b>: %s  <br/>" % (user.name, room.name, cmd), True)
        if user.name in admin and not user.name in owners and not user.name in archknight and not user.name in whitelist:
          room.message("<br/>"+user.name+" Rank 5 [Admin] "+"<br/>"+" Perintah[ ^ atau * ] :<br/>rb , rb2 , wl , whois , webanime , cso , pfpic , mini , prof of profile , gs(Google search) , yt(Youtube) , df(define) , udf(undefine) , fax , bc , sn(sendnote) , rn(readnote) , join , leave , mydict, nick  , staff , setnick , mynick , seenick , profile , rank , myrank , ranker , clear , del , sf , sfc, snc , sfz , myip , mc , mc2 , lock , unlock , lockstatus",True) 
          self.getRoom("mio-chat").message("<br/><br/><b>Nama</b>: %s <br/><b>Rooms</b>: %s  <br/><b>Command</b>: %s  <br/>" % (user.name, room.name, cmd), True)
        if user.name in archwizard and not user.name in owners and not user.name in admin and not user.name in whitelist:
          room.message("<br/>"+user.name+" Rank 4 [ArchWizard] "+"<br/>"+" Perintah[ ^ atau * ] :<br/>rb , rb2 , wl , whois , webanime , cso , pfpic , mini , prof of profile , gs(Google search) , gis(Google Image search) , yt(Youtube) , df(define) , udf(undefine) , fax , bc , sn(sendnote) , rn(readnote), mydict, nick , staff , setnick , mynick , seenick , profile , rank , myrank , ranker , myip , mc(MultiChat) , mc2(MultiChat) , lock , unlock , lockstatus",True)
          self.getRoom("mio-chat").message("<br/><br/><b>Nama</b>: %s <br/><b>Rooms</b>: %s  <br/><b>Command</b>: %s  <br/>" % (user.name, room.name, cmd), True)
        if user.name in archknight and not user.name in owners and not user.name in admin and not user.name in whitelist:
          room.message("<br/>"+user.name+" Rank 3 [Archknight] "+"<br/>"+" Perintah[ ^ atau * ] :<br/>rb , rb2 , wl , whois , webanime , cso , pfpic , mini , prof of profile , gs(Google search) , gis(Google Image search) , yt(Youtube) , df(define) , udf(undefine) , fax , sn(sendnote) , rn(readnote), mydict, nick , staff , setnick , mynick , seenick , profile , rank , myrank , ranker , myip , mc(MultiChat) , mc2(MultiChat) , lock , unlock , lockstatus",True)
          self.getRoom("mio-chat").message("<br/><br/><b>Nama</b>: %s <br/><b>Rooms</b>: %s  <br/><b>Command</b>: %s  <br/>" % (user.name, room.name, cmd), True)
        if user.name in player and not user.name in owners and not user.name in admin and not user.name in whitelist:
          room.message("<br/>"+user.name+" Rank 2 [Player] "+"<br/>"+" Perintah[ ^ atau * ] :<br/>rb , rb2 , wl , webanime , cso , pfpic , mini , prof of profile , gs(Google search) , gis(Google Image search) , yt(Youtube) , df(define) , udf(undefine) , fax , sn(sendnote) , rn(readnote), mydict, nick , staff , setnick , mynick , seenick , profile , rank , myrank , ranker , myip , mc(MultiChat) , mc2(MultiChat) , lock , unlock , lockstatus",True)
          self.getRoom("mio-chat").message("<br/><br/><b>Nama</b>: %s <br/><b>Rooms</b>: %s  <br/><b>Command</b>: %s  <br/>" % (user.name, room.name, cmd), True)
        if user.name in whitelist and not user.name in owners and not user.name in admin and not user.name in archknight:
          room.message("<br/>"+user.name+" Rank 1 [Whitelist] "+"<br/>"+" Perintah[ ^ atau * ] :<br/>rb , rb2 , wl , whois , webanime , cso , pfpic , mini , prof of profile , gs(Google search) , gis(Google Image search) , yt(Youtube) , df(define) , udf(undefine) , sn(sendnote) , rn(readnote), mydict , nick , staff , setnick , mynick , seenick , profile , rank , myrank , ranker , myip , mc(MultiChat) , mc2(MultiChat) , lockstatus",True)
          self.getRoom("mio-chat").message("<br/><br/><b>Nama</b>: %s <br/><b>Rooms</b>: %s  <br/><b>Command</b>: %s  <br/>" % (user.name, room.name, cmd), True)
      ### MultiChat
     ...
     # clear
      ...
      ##Ranker
      if cmd == "ranker":
        room.message("<br/><f x120000FF='0'><b>Owner:</b></f> %s<br/><f x12FF0000='0'><b>Admin:</b></f> %s<br/><f x12216BF5='0'><b>ArchWizard:</b></f> %s<br/><f x12FF00FF='0'><b>ArchKnight:</b></f>%s" % (", ".join(owners), ", ".join(admin), ", ".join(archwizard), ", ".join(archknight)),True)
      ##RankBoard
      if cmd == "rankboard":
	...
	#not really important
      ...
      if cmd == "simlock" and self.getAccess(user) >= 3:
        if args:
          if args in simlock:
            room.message("Already Locked")
          if args in self.roomnames:
            simlock.append(args)
            f = open("simlock.txt", "w")
            f.write("\n".join(simlock))
            f.close()
            room.message("Locked simsimi: %s" % args, True)
        ...
        if not args:
          if room.name not in simlock:
             ...
             f.write("\n".join(simlock))
             f.close()
             room.message("Locked simsimi: %s" % room.name,True)
          else:
             room.message("Already locked :|")
      ##Simi unlock
      if cmd == "simunlock" and self.getAccess(user) >= 3:
        if args:
          if args in simlock and args in self.roomnames:
            ...
            f.write("\n".join(simlock))
            f.close()
            room.message("Unlocked simsimi: %s" % args, True)
          if args not in simlock:
            room.message("Already Unlocked")
        if not args:
          if room.name in simlock:
             ...
             f.write("\n".join(simlock))
             f.close()
             room.message("Unlocked simsimi: %s" % room.name,True)
          else:
             room.message("Already Unlocked :|")
      ##Ping
      if cmd == "ping":
       if not args:
         self.getRoom(room.name).message("@"+" @".join([x.name for x in self.getRoom(room.name).userlist]))
       if args:
         self.getRoom(args).message("@"+" @".join([x.name for x in self.getRoom(args).userlist]))
      ## cso
      if cmd=="cso":
       if len(args)>0:
          offline = None
      ...
      ## upTime
      elif cmd == "shutdown" or cmd == "sesson" and self.getAccess(user) >= 2:
    ...
    ###### Define            
        ...
        if not args:
            if user.name in owners and not user.name in whitelist:
              room.message(user.name+" Kamu Rank 6 [Owner] ",True)
              ...
              room.message(user.name+" Kamu Rank 5 [Admin] ",True)
              ...
              room.message(user.name+" Kamu Rank 2 [Player]",True)
        ...
        if args:
              sss = args
              if sss in owners:
                  room.message(sss.title()+" Rank Dia 6 [Owner] ",True)
              if sss in admin:
                  room.message(sss.title()+" Rank Dia 5 [Admin] ",True)
              if sss in archwizard:
                  room.message(sss.title()+" Rank Dia 4 [ArchWizard] ",True)
              if sss in archknight:
                  room.message(sss.title()+" Rank Dia 3 [ArchKnight] ",True)
              if sss in player:
                  room.message(sss.title()+" Rank Dia 2 [Player] ",True)
              if sss in whitelist:
          ...
          try:
           ...
           if args in self.roomnames:
            if user.name in owners or user.name in admin or user.name in archknight or user.name in archwizard:
              ...
              f.write("\n".join(locks))
              f.close()
              room.message("Locked: %s" % args, True)
           ...
           if args not in self.roomnames:
            room.message("im not there :|")
            return
          except:
            room.message("Something wrong",True)
          ...
          try:
           if not args:
             ...
             if rnm in locks:
               ...
               room.message("bahkan sudah diunlock")
           if args:
             ar = args
             if ar in locks:
               locks.remove(ar)
               ...
               room.message("Bahkan itu sudah unlock")
          except:
           room.message("Sesuatu yang salah",True)
    ...
    ##### Whitelist
        ...
        if name not in whitelist and name not in archwizard and name not in player and name not in owners and name not in admin and name not in archknight and name not in blacklist:
          room.message("Sekarang "+args+" dapat menggunakan Mio")
          whitelist.append(name)
          f = open("whitelist.txt","w")
          f.write("\n".join(whitelist))
          f.close
     ...
     ###blacklist
      ...
      if cmd == "uwl" and self.getAccess(user) >= 5:
        try:
          if args in owners:
            room.message("Ma'af Mio tidak dapat melakukannya")
          if args in admin:
            admin.remove(args)
            f = open("admin.txt","w")
            f.write("\n".join(admin))
  ...
  def onJoin(self, room, user):
    if room.name == "mio-chat": room.message("Welcome in This Room @"+user.name+ "Having Fun Yeah... XD")
    print(user.name + " joined the chat!")
  
  def onLeave(self, room, user):
    if room.name == "mio-chat": room.message("@"+user.name+ "Left This Room :(")
  ...
  def onPMMessage(self, pm, user, body):
    ...
    self.pm.message(ch.User("nosoba"), "Incoming PM From ["+user.name+"] - "+body)
    self.setTimout(int(1), self.getRoom("mio-chat").message, "Incoming PM From ["+user.name+"] - "+body)
